...
addChild c (Element name attrs cs)
 = Element name attrs (cs ++ [c])
...
popAndAdd (el:(Element name attrs childs):stk)
  = Element name attrs (childs ++ [el]) : stk
...
parseAttributes input
  ...
  where
    ...
    startVal = (tail . skipSpace . tail . skipSpace) rest
    (attVal, rest') = break (=='\"') startVal
    -- tail to skip the ending "
